...
test_files= [r'PMI-Sum-T-Bigram-Question-Stem.csv', r'PMI-Sum-T-Trigram-Question-Stem.csv']

df_ac_lemma_q = pd.read_csv(data_dir + r'Lemma-Question.csv')
df_ac_lemma_q = df_ac_lemma_q.set_index('AC_Doc_ID')

df_ac_lemma_q_stem = df_ac_lemma_q[df_ac_lemma_q['Pre_Col_Name'].isin(['Stem'])]
df_ac_sum_t_lemma_q_buf, lemma_sum_total = ac_term_proportion(df_ac_lemma_q_stem, Lemma_count_start_from_question - 1)

df_ac_bigram_q = pd.read_csv(data_dir + r'Bigram-Question.csv')
df_ac_bigram_q = df_ac_bigram_q.set_index('AC_Doc_ID')

df_ac_bigram_q_stem = df_ac_bigram_q[df_ac_bigram_q['Pre_Col_Name'].isin(['Stem'])]
df_ac_sum_t_bigram_q_buf = ac_bi_trigram_pmi(df_ac_bigram_q_stem, Ngram_count_start_from_question - 1,
                                df_ac_sum_t_lemma_q_buf, lemma_sum_total, 'bigram', Decimal_places)
df_ac_sum_t_bigram_q_buf = df_ac_sum_t_bigram_q_buf[df_ac_sum_t_bigram_q_buf['Bigram_sum'] >= 1]
...
df_ac_trigram_q_stem = df_ac_trigram_q[df_ac_trigram_q['Pre_Col_Name'].isin(['Stem'])]
df_ac_sum_t_trigram_q_buf = ac_bi_trigram_pmi(df_ac_trigram_q_stem, Ngram_count_start_from_question - 1,
                                df_ac_sum_t_lemma_q_buf, lemma_sum_total, 'trigram', Decimal_places)
df_ac_sum_t_trigram_q_buf = df_ac_sum_t_trigram_q_buf[df_ac_sum_t_trigram_q_buf['Trigram_sum'] >= 1]
